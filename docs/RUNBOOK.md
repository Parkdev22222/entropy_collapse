...
| **★ GPU 스택만 손으로 깔면 나머지 의존성이 통째로 빠진다** | 2026-09-23 새 H100 파드가 `vllm`·`ray`·`transformers`·`flash-attn`만 plain pip으로 받고 `setup_env.sh`를 건너뛰었다. `signed s4`가 시작 직후 죽었고 큐는 다음 arm으로 넘어갔다 — 원인은 **tensorboard 미설치**다. 모든 arm이 `trainer.logger=['console','tensorboard']`로 돌고(`_arms.sh:345`, `run_grpo.sh:207`, `run_uniform_ablation.sh:170`) verl은 그걸 **트레이너 초기화에서** 여는데, 그건 step 1 전이라 로그만 보면 그 arm이 실패한 것처럼 보인다. 그리고 빠진 건 그 하나가 아니다 — `_check_deps.py`가 아는 목록이 18개이고 트레이너가 제일 먼저 연 것이 tensorboard였을 뿐이다. **`bash run/setup_env.sh`를 한 번 돌린다**(`INSTALL_GPU_STACK=1`이 없으면 vllm·ray는 안 건드린다). `env_preflight`이 이제 이 import를 큐 시작 전에 해본다 |
...
`bash run/setup_env.sh` → `check_env_pins.py` → flash-attn 재설치(아래 §flash-attn) →
`env_preflight` → `bash run/migrate_pod.sh --check` → `bash run/run_status.sh`.
`--check`가 실패하면 MTP 헤드·보정이 안 온 것이고, 그 상태로는
`run_uniform_ablation.sh:149`가 REFUSE해서 **tree arm이 하나도 안 뜬다.**
---
