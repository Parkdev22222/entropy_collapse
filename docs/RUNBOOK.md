...
| **ray 가 핀이 없다** | `requirements.txt:16`·`setup_env.sh`가 `ray[default]`를 무핀으로 깐다. vllm은 0.8.4로 박혀 있는데 그 짝인 ray는 떠다녀서, 새 파드가 `ray 2.58.0`을 받았다. vllm 0.8.4는 `opentelemetry-api<1.27`을 요구하고 ray 2.58은 `Meter.create_histogram(explicit_bucket_boundaries_advisory=)`를 부른다 — **양립 불가**. `RAY_PIN`을 **학습되는 박스의 `ray.__version__`으로** 고정해라 |
