...
### 2.1 "(L)만으로 충분하다"는 반박을 먼저 세운다

식 (2)만으로는 (V)가 **실질적으로** 중요하다는 근거가 못 된다. 자연스러운 반박이 있다:

> 분기점은 정책이 불확실한 자리, 즉 엔트로피가 높은 자리다. 그런데 $\Omega$ 는 엔트로피가 높은 곳에서 크게 나올 테니, 로컬 항만으로도 분기점은 자동으로 보호되는 것 아닌가?

**반박의 전제는 옳다.** Phase 1 워밍업 롤아웃 600개 cut point에서 분기 다양성(`branch_diversity`, 연속열들의 의미 임베딩 분산)과 국소 엔트로피는 확실히 함께 움직인다:

| 상관 | Pearson | Spearman |
|---|---|---|
| $H_{t+1}$ vs 분기 다양성 | +0.419 | **+0.645** |
| $\sum_k H_{t+k}$ (entropy-to-go) vs 분기 다양성 | +0.529 | **+0.820** |

`docs/phase1_results_Qwen2.5-Math-1.5B-paper.json`, $n=600$. 분기점이 고엔트로피 자리라는 것은 이 모델에서 실측으로 성립한다.

무너지는 것은 **다음 고리**다.

### 2.2 $\Omega$ 는 엔트로피의 *수준*이 아니라 surprisal의 *편차*를 본다

$\Omega$ 의 부호 결정 인자 $\ln\pi_a + H$ 를 surprisal $I_a := -\ln \pi_a$ 로 다시 쓰면 (§6, `STEERF_method.md` §3):

$$
\Omega \;=\; A\,(1-\pi_a)\,\big(I_a - H\big)
\tag{2a}
$$

그런데 엔트로피의 정의가 $H = \mathbb{E}_{a\sim\pi}[I_a]$ 다. 즉 $I_a - H$ 는 **정의상 평균이 0인 요동**이며, $\lvert\Omega\rvert$ 의 크기를 정하는 것은 그 요동의 *폭*이지 엔트로피의 *높이*가 아니다.

**따름정리 (균등 분포에서 $\Omega$ 는 항등적으로 0).** 후보 $k$ 개에 균등하면 모든 $a$ 에 대해 $I_a = \ln k = H$ 이므로 $\Omega \equiv 0$. 여러 갈래가 비슷한 확률인 자리 — 가장 분기점다운 자리 — 에서 로컬 항의 보호가 정확히 사라진다.

### 2.3 그리고 그 $0$ 은 오류가 아니라 정답이다

이것이 이 논증의 핵심 고리다. 균등 분포는 엔트로피의 **최대점**이므로 $\partial H/\partial z_a = 0$ 이 모든 $a$ 에 대해 성립한다 — $\Omega$ 가 틀린 것이 아니라 맞는 답을 내고 있다. 따라서 **로컬 항을 개선해서 이 사각지대를 메울 수 없다.**

균등 분포에서 로짓 하나만 $\delta$ 만큼 움직였을 때의 엔트로피는 닫힌 형태로 풀린다. $S := e^{\delta} + k - 1$ 로 두면

$$
H(\delta) \;=\; \ln S \;-\; \frac{\delta\, e^{\delta}}{S},
\qquad
...
$H'$ 의 부호가 $-\operatorname{sign}(\delta)$ 이므로 **$\delta \ne 0$ 이면 어느 방향으로 밀든 엔트로피는 반드시 감소한다.** 다만 그 감소는 전적으로 2차항이다:

| 분포 | $H_0$ | $\Omega$ (1차 예측) | 실제 $\Delta H$ ($\delta=0.1$) | $\Delta H$ ($\delta=0.3$) |
|---|---|---|---|---|
| 10갈래 균등 | 2.303 | $0$ | **−0.00047** | −0.00474 |
| 50갈래 균등 | 3.912 | $0$ | −0.00010 | −0.00107 |
| top .5 + 9균등 | 1.792 | −0.549 | −0.0561 | −0.1747 |
| top .9 + 9균등 | 0.545 | −0.396 | −0.0384 | −0.1087 |

즉 **국소 엔트로피 손상만 놓고 보면 균등 분기점은 실제로 안전하다** (뾰족한 자리의 1/120). 로컬 항이 거기에 예산을 쓰지 않는 것은 국소 목적함수에 대해 올바른 배분이다.

궤적 엔트로피에 대해서는 그렇지 않다.

### 2.4 두 채널의 크기는 서로 반대로 정렬된다

방문 항의 전치인자는 $(1-\pi_a)$ 다 (식 (7)). 이것은 분포가 평평할수록 $1$ 에 수렴한다. 그리고 §2.1의 실측에 의해 $A_H$ 역시 분기 다양성과 함께 커진다. **두 인자가 같은 방향으로 커지고, 그 방향은 $\Omega \to 0$ 인 방향이다.**
...
(분포족: top 토큰 $q$, 잔여 질량 $1-q$ 를 $m$ 개에 균등. $\mathbb{E}\lvert\Omega\rvert$ 는 그 위치에서 실제로 표본된 토큰에 대한 기댓값.)

$\lvert\Omega\rvert$ 를 독점하는 것은 **"모델이 꽤 확신하는데 꼬리 토큰이 뽑힌"** 자리다. 그런 자리는 국소 엔트로피 손상은 크지만 형제들이 갈라질 확률은 중간(0.57)이다. 반대로 분기 확률이 $1.0$ 인 자리에서 로컬 항은 $0$ 을 쓰고 방문 전치인자는 최대가 된다.

### 2.5 그래서 (V) 가 필요한 이유는 이렇게 말해야 한다

> 로컬 항은 방문 채널에 대해 **blind일 뿐 아니라 anti-correlated** 다. 감쇠 예산을 국소 엔트로피 손상이 큰 자리(뾰족한 분포에서의 희귀 토큰 표본)에 몰아주고, 궤적 엔트로피 손상이 최대인 자리(여러 갈래가 비슷한 확률인 진짜 분기점)에는 정확히 $0$ 을 쓴다. 그리고 그 $0$ 은 1차 근사의 **정답**이므로 (식 (2b)) 로컬 항을 손봐서 고칠 수 없다 — 다른 채널이 필요하다.

$A_H$ 는 현재 분포의 모양에 전혀 의존하지 않고 형제들의 *미래*에만 의존하므로 (식 (4)), $\Omega$ 가 소멸하는 바로 그 자리에서 살아 있다. 두 항은 경쟁 관계가 아니라 상보 관계다.

> **아직 측정되지 않은 것.** §2.4의 두 인자 중 $(1-\pi_a)$ 는 위 분포족에 대한 계산이고 $A_H$ 는 §2.1의 상관에서 온 간접 근거다. **둘의 곱을 실제 롤아웃에서 직접 잰 적이 없다.** 필요한 측정은 하나다: 진짜 sibling divergence 위치에서의 평균 $\lvert\Omega\rvert$ 순위 대 나머지 위치. 그 수치가 나오기 전까지 §2.4는 예측이지 관측이 아니다.
...
**더 근본적으로, G2는 §2의 주장을 검증할 수 없다.** §2.3이 보인 것은 평평한 분기점에서 *국소* 엔트로피가 애초에 2차로만 움직인다는 것이다 (뾰족한 자리의 1/120). 그러므로 국소 엔트로피 갭이 움직이지 않는다는 관측은 (V) 채널에 대해 아무것도 말해주지 않는다 — 주장 자체가 "손상은 국소가 아니라 궤적 수준에서 일어난다"이기 때문이다. §2를 검증하려면 궤적 수준 지표(형제 subtree 생존율, 분기 이후 다양성, pass@k)가 필요하며, 이 저장소에는 아직 그런 지표가 없다.
