...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            # Restore original configuration passing method
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            #  handle clip position information
            clip_positions_summary = {}
            if clip_positions:
                # Merge all clip position information
                total_clip_high_count = sum(pos.get("clip_high_count", 0) for pos in clip_positions if pos)
                total_clip_low_count = sum(pos.get("clip_low_count", 0) for pos in clip_positions if pos)
                total_clipped_count = sum(pos.get("total_clipped_count", 0) for pos in clip_positions if pos)
                
                clip_positions_summary = {
                    "total_clip_high_count": total_clip_high_count,
                    "total_clip_low_count": total_clip_low_count,
                    "total_clipped_count": total_clipped_count,
                }
            

            output = DataProto(meta_info={"metrics": metrics, "clip_positions": clip_positions_summary})
