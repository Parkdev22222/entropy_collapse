...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if "reward_extra_info" in result:
                for key, lst in result["reward_extra_info"].items():
                    # Filter out None values before adding to reward_extra_infos_dict
                    filtered_lst = [x if x is not None else 0.0 for x in lst]  # Replace None with 0.0
                    reward_extra_infos_dict[key].extend(filtered_lst)
                    print(f"len reward_extra_infos_dict['{key}']: {len(reward_extra_infos_dict[key])}")
                    # Log if any None values were found
                    none_count = sum(1 for x in lst if x is None)
                    if none_count > 0:
                        print(f"Warning: reward_extra_info['{key}'] contained {none_count} None values, replaced with 0.0")
        ...
        for key_info, lst in reward_extra_infos_dict.items():
            assert len(lst) == 0 or len(lst) == len(sample_scores), f"{key_info}: len(lst)={len(lst)}, len(sample_scores)={len(sample_scores)}"
            # Add debug information for None values
            none_count = sum(1 for x in lst if x is None)
            if none_count > 0:
                print(f"Warning: {key_info} contains {none_count} None values out of {len(lst)} total values")
        ...
        # Save detailed evaluation results to JSON file only in val_only mode
        if self.config.trainer.get("val_only", False):
            self._save_evaluation_results(data_src2var2metric2val, sample_inputs, sample_outputs, sample_scores)

        return metric_dict

    def _save_evaluation_results(self, data_src2var2metric2val, sample_inputs, sample_outputs, sample_scores):
        """Save detailed evaluation results to JSON file, consistent with existing result file format"""
        import json
        from datetime import datetime
        
        try:
            # Create results directory - use environment variable or default to ./evaluation_results
            eval_results_dir = os.environ.get('EVAL_RESULTS_DIR', './evaluation_results')
            os.makedirs(eval_results_dir, exist_ok=True)
            
            # Generate filename - consistent with existing format
            model_name = os.path.basename(self.config.actor_rollout_ref.model.path)
            current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"{model_name}_{current_time}.json"
            filepath = os.path.join(eval_results_dir, filename)
            
            # Build result data structure - consistent with existing format
            result_data = {
                "timestamp": datetime.now().isoformat(),
                "description": "VERL multi-dataset evaluation results",
                "filtering_rules": {
                    "aime_2024_dapo_boxed": "Keep only @32 results",
                    "aime_2025_dapo_boxed": "Keep only @32 results",
                    "other_datasets": "Keep only @1 results"
                },
                "results": {}
            }
            ...
            for data_source, var2metric2val in data_src2var2metric2val.items():
                result_data["results"][data_source] = {}
                
                for var_name, metric2val in var2metric2val.items():
                    for metric_name, metric_val in metric2val.items():
                        # Save all important metrics
                        if any(metric_name.startswith(pfx) for pfx in ["mean", "maj", "best", "worst", "std"]):
                            # Ensure correct data types
                            if isinstance(metric_val, (int, float, np.integer, np.floating)):
                                result_data["results"][data_source][metric_name] = float(metric_val)
                            else:
                                result_data["results"][data_source][metric_name] = metric_val
            ...
            print(f"📊 Evaluated {len(data_src2var2metric2val)} datasets, {len(sample_inputs)} samples in total")
            
            # Print main metrics summary
            print("\n🎯 Main evaluation metrics summary:")
            print("=" * 70)
            
            total_datasets = len(data_src2var2metric2val)
            for i, (data_source, metrics) in enumerate(result_data["results"].items(), 1):
                print(f"\n📈 [{i}/{total_datasets}] {data_source}:")
                
                # Find main accuracy metrics
                acc_metrics = {k: v for k, v in metrics.items() if 'acc' in k and 'mean' in k}
                if acc_metrics:
                    for metric_name, metric_val in acc_metrics.items():
                        print(f"  {metric_name}: {metric_val:.4f} ({metric_val*100:.2f}%)")
                else:
                    # If no acc metrics, show reward metrics
                    reward_metrics = {k: v for k, v in metrics.items() if 'reward' in k and 'mean' in k}
                    for metric_name, metric_val in reward_metrics.items():
                        print(f"  {metric_name}: {metric_val:.4f}")
                
                # Show other important metrics
                other_metrics = {k: v for k, v in metrics.items() 
                               if any(k.startswith(pfx) for pfx in ["best", "maj", "std"]) and 'mean' not in k}
                if other_metrics:
            ...
            print("=" * 70)
            print(f"📁 Detailed results file: {filepath}")
            print(f"⏰ Evaluation completion time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def _save_checkpoint(self, metrics=None):
        ...
        if self.config.trainer.get("save_best_only", False) and metrics is not None:
            ...
            if os.path.exists(best_checkpoint_info_path):
                try:
                    with open(best_checkpoint_info_path, 'r') as f:
                        best_info = json.load(f)
                        best_metric_value = best_info.get("best_metric_value", float('-inf'))
                        best_checkpoint_step = best_info.get("best_checkpoint_step", None)
                except:
                    pass
            
            if current_metric_value > best_metric_value:
                print(f"🎉 New best {best_metric_key}: {current_metric_value:.4f} (previous: {best_metric_value:.4f})")
                
                # If enabled to delete old checkpoint, delete previous best checkpoint
                if self.config.trainer.get("delete_old_best_checkpoint", False) and best_checkpoint_step is not None:
                    old_checkpoint_path = os.path.join(self.config.trainer.default_local_dir, f"global_step_{best_checkpoint_step}")
                    if os.path.exists(old_checkpoint_path):
                        import shutil
                        try:
                            shutil.rmtree(old_checkpoint_path)
                            print(f"Deleted old best checkpoint: {old_checkpoint_path}")
                        except Exception as e:
                            print(f"Warning: Failed to delete old checkpoint {old_checkpoint_path}: {e}")
                
                # Update best value
                best_metric_value = current_metric_value
                best_checkpoint_step = self.global_steps
            else:
                print(f"Skipping checkpoint save. Current {best_metric_key}: {current_metric_value:.4f} (best: {best_metric_value:.4f})")
                return  # Do not save checkpoint
        ...
        if self.config.trainer.get("save_best_only", False):
            ...
            best_checkpoint_info = {
                ...
                "last_updated_step": self.global_steps
            }
    ...
    def fit(self):
        ...
        if self.val_reward_fn is not None and self.config.trainer.get("val_before_train", True):
            ...
            assert val_metrics, f"val_metrics={val_metrics}"
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                metrics = {} # Initialize dictionary for storing current batch metrics
                timing_raw = {} # Initialize dictionary for storing current batch timing
                batch: DataProto = DataProto.from_single_dict(batch_dict)

                # 1. rollout stage
                # pop those keys for generation
                # Separate data for rollout from batch (input_ids, attention_mask, position_ids, etc.), keep other data for subsequent processing.
                batch_keys_to_pop = ["input_ids", "attention_mask", "position_ids"]
                non_tensor_batch_keys_to_pop = ["raw_prompt_ids"]
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        if not self.async_rollout_mode:
                            ...
                            # Call generate_sequences method of actor_rollout_wg (Actor-Rollout Worker Group) to generate response sequences
                            gen_batch_output = self.async_rollout_manager.generate_sequences(gen_batch)

                        timing_raw.update(gen_batch_output.meta_info["timing"])
                        gen_batch_output.meta_info.pop("timing", None)
                    
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.REMAX: # GRPO does not use this part
                    ...
                    # Generate a unique ID (uid) for each sample in the batch
                    batch.non_tensor_batch["uid"] = np.array([str(uuid.uuid4()) for _ in range(len(batch.batch))], dtype=object)
                    # repeat to align with repeated responses in rollout
                    batch = batch.repeat(repeat_times=self.config.actor_rollout_ref.rollout.n, interleave=True)
                    batch = batch.union(gen_batch_output) # Merge generated sequence data (gen_batch_output) back into original batch data (batch)
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        if self.config.reward_model.launch_reward_fn_async:
                            ...
                            reward_tensor, reward_extra_infos_dict = compute_reward(batch, self.reward_fn) # GRPO computes reward here
                            # reward_tensor, [train_batch_size*rollout.n, max_response_length]

                            row_max_values, max_indices = reward_tensor.max(dim=1)
                    ...
                    with marked_timer("old_log_prob", timing_raw, color="blue"):
                        ...
                        max_prob_log_probs = old_log_prob.batch["max_prob_log_probs"]  # get max_prob_log_probs
                        response_masks = batch.batch["response_mask"]
                        loss_agg_mode = self.config.actor_rollout_ref.actor.loss_agg_mode
                        entropy_agg = agg_loss(loss_mat=entropys, loss_mask=response_masks, loss_agg_mode=loss_agg_mode) # scalar
                        # Additionally compute entropy for seq-mean-token-sum mode
                        # seq_entropy_agg = agg_loss(loss_mat=entropys, loss_mask=response_masks, loss_agg_mode="seq-mean-token-sum") # scalar
                        old_log_prob_metrics = {
                            "actor/entropy": entropy_agg.detach().item(),
                            # "actor/seq_entropy": seq_entropy_agg.detach().item()
                        }
                    ...
                    # ref model computation
                    if self.use_reference_policy:
                        # compute reference log_prob
                        with marked_timer("ref", timing_raw, color="olive"):
                            if not self.ref_in_actor: # only valid with lora
                                ref_log_prob = self.ref_policy_wg.compute_ref_log_prob(batch)
                            else:
                                ref_log_prob = self.actor_rollout_wg.compute_ref_log_prob(batch)
                            batch = batch.union(ref_log_prob)

                    # compute values for PPO
                    if self.use_critic:
                        with marked_timer("values", timing_raw, color="cyan"):
                            values = self.critic_wg.compute_values(batch)
                            batch = batch.union(values)

                    # 4. compute final advantage function, including kl div calculation
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        batch = compute_advantage( # Key step for computing advantage function
                            batch,
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        # detailed debug info - check original metrics
                        original_metrics = actor_output.meta_info["metrics"]
                        
                        
                        # use reduce_metrics to handle distributed worker metrics
                        actor_output_metrics = reduce_metrics(original_metrics)
                    ...
                    if self.config.trainer.save_freq > 0 and (is_last_step or self.global_steps % self.config.trainer.save_freq == 0 or esi_close_to_expiration):
                        ...
                        with marked_timer("save_checkpoint", timing_raw, color="green"):
                            # Collect current step metrics for best checkpoint judgment
                            current_metrics = {}
                            if self.val_reward_fn is not None and self.config.trainer.test_freq > 0 and (is_last_step or self.global_steps % self.config.trainer.test_freq == 0):
                                current_metrics.update(val_metrics)
                            
                            self._save_checkpoint(metrics=current_metrics)
