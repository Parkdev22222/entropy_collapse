...
# Define ActorConfig type (if not exists)
try:
    from verl.workers.actor.base import ActorConfig
except ImportError:
    ActorConfig = None
...
def compute_policy_loss_with_entropy(
    ...
    Args:
        ...
        linear (bool, optional):
            ...
            - True: Use linear mapping to [token_weight_min, token_weight_max]
            - False: Use exponential mapping token_weights = exp(-k * metric)
        entropy_control_mode (str, optional):
            "symmetric" (default, paper) or "asymmetric". See
            `compute_token_weights` for details.

    Returns:
        tuple: A tuple containing:
            - policy_loss (torch.Tensor): The computed policy loss
            - clipfrac (torch.Tensor): Fraction of clipped probabilities
            - kl_div (torch.Tensor): KL divergence between old and new policies
            - clipfrac_lower (torch.Tensor): Fraction of lower clipped probabilities
            - clipfrac_high (torch.Tensor): Fraction of upper clipped probabilities
            - clipfrac_low (torch.Tensor): Fraction of lower clipped probabilities
            - clipped_by_high (torch.Tensor): Boolean tensor indicating high clipping
            - clipped_by_low (torch.Tensor): Boolean tensor indicating low clipping
            - clip_stats (dict): Dictionary containing clipping and weighting statistics
    """
    ...
    # Detect clipping due to clip_high (ratio > 1 + cliprange_high)
    clipped_by_high = (ratio > (1 + cliprange_high)) & (advantages > 0)  # Positive advantage and ratio too large
    pg_clipfrac_high = verl_F.masked_mean(clipped_by_high.float(), response_mask)
    
    # Detect clipping due to clip_low (ratio < 1 - cliprange_low)
    clipped_by_low = (ratio < (1 - cliprange_low)) & (advantages < 0)  # Negative advantage and ratio too small
    pg_clipfrac_low = verl_F.masked_mean(clipped_by_low.float(), response_mask)
    ...
    # Only count token_weights on valid tokens (response_mask>0)
    valid = response_mask > 0
    token_weights_mean = token_weights[valid].mean().item()
    token_weights_min = token_weights[valid].min().item()
    token_weights_max = token_weights[valid].max().item()
    ...
    clip_stats = {
        ...
        "token_weights_mean": token_weights_mean,
...
def compute_policy_loss(
    ...
    clipped_by_high = (ratio > (1 + cliprange_high)) & (advantages > 0)
    pg_clipfrac_high = verl_F.masked_mean(clipped_by_high.float(), response_mask)
    

    clipped_by_low = (ratio < (1 - cliprange_low)) & (advantages < 0)
    pg_clipfrac_low = verl_F.masked_mean(clipped_by_low.float(), response_mask)
...
def compute_policy_loss_clip_cov(
    ...
    Args:
        ...
        config (dict, optional):
            Configuration containing clip_cov parameters.
    """
...
def compute_policy_loss_gspo(
    ...
    response_mask: torch.Tensor,
    loss_agg_mode: str = "token-mean",
    config: Optional[Union[DictConfig, ActorConfig]] = None,
