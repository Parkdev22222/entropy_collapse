...
per run), averaged over $n$ seeds. \emph{uplift} is $\mathrm{maj@}32 - \mathrm{acc@}32$. Entropy is the token-mean quantity discussed in
...
Run-to-run spread is itself worth reporting. Across our seeds the standard deviation
of an arm's converged-window accuracy is \num{Seedsd}, and of a paired difference
\num{Pairedsd}. For scale, two validations of the \emph{same} weights on these $30$
questions differ by about $.004$, so seed variation is the larger term by roughly an
order of magnitude, and any comparison quoted at single-run precision is quoting
noise.
