...
| Qwen2.5-Math-7B | 116 GiB | 52 GiB | ⚠️ `OFFLOAD=1` 필수 | ⚠️ **`OFFLOAD=1` 필수** |
| Qwen2.5-14B / Coder-14B | 215 GiB | 92 GiB | ❌ (4장 필요) | ⚠️ `OFFLOAD=1`로 가능 |

> **7B는 H200 1장에서도 `OFFLOAD=1` 없이는 불가능하다.** 학습 단계만 보면
> 116 GiB로 아슬아슬하게 들어가지만, **생성 단계**에서 AdamW 85 GiB + 가중치
> 14 GiB가 GPU에 남은 채 vLLM 풀이 얹히기 때문이다. `GPU_MEM_UTIL`을 0.1까지
> 낮춰도 127 GiB로 여전히 넘친다 — 상주분 99 GiB가 이미 예산의 대부분이라
> **vLLM 풀을 줄이는 것으로는 해결되지 않는다.** `OFFLOAD=1`이면 생성 60 GiB /
> 학습 52 GiB로 양쪽 다 여유롭다.
...
7B(Table 3)는 A100·H200 모두 `OFFLOAD=1`이 필요하고, H200 쪽이 메모리 여유는
훨씬 크다. 14B와 코드 3종(Table 4·5)은 오프로드를
