...
class SequentialMTPHeads(nn.Module):
    ...
    def forecast_entropy(
        ...
        n_check = self.num_heads if kappa is None else int(kappa)
        if isinstance(temperature, (int, float)):
            temps = [float(temperature)] * n_check
        else:
            temps = [float(t) for t in temperature][:n_check]
            if len(temps) < n_check:
                raise ValueError(f"temperature sequence too short: {len(temps)} < {n_check}")
