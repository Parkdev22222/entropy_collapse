...
Kept out of the verl tree on purpose.  The edits inside ``verl/`` stay small
and mechanical (see ``git diff`` against the upstream-import commit);
everything with real logic lives here, where it can be tested on a CPU with
no verl runtime installed.
...
def forecast_h_togo(
    ...
    if calib is not None:
        # Per-head calibration temperature, folded into ONE forecast pass.
        # (The earlier per-head-call spelling — forecast_entropy(kappa=k+1)[k]
        # in a loop — recomputed heads 0..k-1 every iteration: kappa(kappa+1)/2
        # head evaluations instead of kappa, ~2.5x the unembedding cost at
        # kappa=4, and each evaluation walks [B*T, V].)
        temps = [temperature * float(calib.temperature[k]) for k in range(cfg.kappa)]
    else:
        temps = temperature
    head_entropies = heads.forecast_entropy(
        h, lm_head, kappa=cfg.kappa, chunk_size=chunk_size, temperature=temps
    )
