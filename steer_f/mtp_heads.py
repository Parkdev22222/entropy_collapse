    ...
    def forecast_entropy(
        ...
        Args:
            ...
            temperature: divide logits before the softmax.  Applied *inside*
                ...
                A sequence gives head ``k`` its own temperature in this same
                single pass — the per-head calibration path.  (Calling this
                method once per head with ``kappa=k+1`` instead recomputes
                heads ``0..k-1`` every time, ``K(K+1)/2`` head evaluations for
                ``K`` needed — measured as the dominant cost of the forecast
        ...
        if isinstance(temperature, (int, float)):
            temps = [float(temperature)] * n_heads
        else:
            temps = [float(t) for t in temperature]
            if len(temps) < n_heads:
                raise ValueError(
                    f"temperature sequence has {len(temps)} entries, need {n_heads}"
                )
            temps = temps[:n_heads]
        if any(t <= 0 for t in temps):
            raise ValueError(f"all temperatures must be > 0, got {temps}")
