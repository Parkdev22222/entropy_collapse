...
# silently resolving to "Qwen/Qwen2.5-Math-Llama-3.2-3B-Instruct".
backbone_profile () {   # <tag> -> sets MODEL_PATH / VAL_PARQUET / BEST_METRIC_KEY
    ...
    case "${tag}" in
        ...
        Llama-3.2-3B-Instruct)
            ...
            #
            # 2026-09-17: instruct, not the base checkpoint this row started as.
            # The base one was measured on this protocol and scored .020 on
            # MATH500 mean@1, .024 on GSM8K -- grade-school arithmetic. Every
            # sample came back as dapo:[INVALID]: the model continues the prompt
            # instead of answering it. The cause is that Meta ships a chat
            # template with the BASE tokenizer, and both scripts/_common.py's
            # apply_chat_template() and verl's RLHFDataset apply a template when
            # one is present, so a model that was never instruction-tuned gets
            # handed the instruction format. Qwen2.5-Math hides this: its
            # pretraining already answers a bare prompt with \boxed{}.
            #
            # A 2% solve rate is not a weak row, it is no row at all. GRPO gives
            # a group zero advantage when its eight rollouts score alike, and
            # .98^8 = .85 of the groups come back all-wrong -- on DAPO-Math-17k,
            # which is harder than MATH500, more than that. Before committing a
            # backbone here, gate it on the training-set group pass rate with
            # scripts/phase3_port_model.py passrate, not on a validation score:
            # the validation set says whether we can MEASURE the backbone, the
            # group pass rate says whether it can LEARN.
